  ...
  async checkAvailability(
    ...
    try {
      ...
      for (const item of response.data as HolidayParkAvailability[]) {
        ...
        if (resortIds && resortIds.length > 0 && !resortIds.includes(item.resort_id)) {
          continue;
        }

        // Filter by accommodation type if specified
        if (accommodationTypeIds && accommodationTypeIds.length > 0 && !accommodationTypeIds.includes(item.accommodation_type_id)) {
          continue;
        }
